...
GLuint BladeList;
GLuint LocateView; // Look the position on different ways
...
Display( )
{
	...
	// Add Code: change inside eye position
	...
	glRotatef(BladeAngle, 0., 1., 0. ); //X-Z plane
	glScalef(5.0, 1., 1.); // radius 5.0
	glRotatef(90., 1., 0., 0.);
	...
	glRotatef(BladeAngle*3 , 1., 0., 0.); //Y-Z plane ,  the smaller one must rotate 3X as fast as the larger one
	glScalef(1., 1., 1.5); // radius 1.5
	glRotatef(90., 0., 1., 0.);
	glCallList(BladeList);
	glPopMatrix();
	...
	glColor3f( 1., 1., 1. );
	DoRasterString( 0., 1., 0., "Helicopter" );
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., "Project 2_YuWenTseng" );
