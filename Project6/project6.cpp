...
const char *WINDOWTITLE = { "Project6 - YuWenTseng" };
...
//Add Code
enum ViewPosition  // Set the outside and the inside view
{
	OUTSIDE,
	INSIDE
};
...
Display( )
{
    ...
    glPushMatrix();
        ...
        for(int c = 0; c < numcurves; c++){
            ...
            float move = (sin(Time*2*M_PI)+1)/2;
            struct Curve curve;
            struct Point p0 = {0., 0., 0.};
            struct Point p1 = {cos(angle)*2, 1., sin(angle)*2};
            struct Point p2 = {cos(angle/3)*move*5, 2.-move*2, sin(angle*3)*move*4};
            struct Point p3 = {cos(angle/2)*move*6, 3.-move*3, sin(angle/2)*move*6};
            curve.p0 = p0;
            curve.p1 = p1;
            curve.p2 = p2;
            curve.p3 = p3;
    ...
    glPushMatrix();
        ...
        for(int c = 0; c < numcurves; c++){
            ...
            struct Point p3 = {cos(angle)/3, -3.*(sin(angle)+1), sin(angle)/1};*/
            struct Point p1 = {cos(angle)/5, -3., sin(angle)/5};
            struct Point p2 = {sin(angle)/4, -3., cos(angle)/4};
            struct Point p3 = {cos(angle)/3, -3.*(sin(angle)+2), sin(angle)/2};
            curve.p0 = p0;
            curve.p1 = p1;
            curve.p2 = p2;
            curve.p3 = p3;
    ...
    glLoadIdentity( );
    glColor3f( 1., 1., 1. );
    DoRasterString( 5., 5., 0., "Project6 - YuWenTseng" );
