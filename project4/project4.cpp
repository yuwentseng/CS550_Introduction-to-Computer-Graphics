...
const char *WINDOWTITLE = { "Project4 - YuWenTseng" };
...
Display( )
{
	...
	//light 1
	//SetSpotLight(GL_LIGHT1, 10.*cos(angle / 32.), 0., 10.*sin(angle / 32.), -cos(angle / 32.), 0., -sin(angle / 32.), 1., 1., 0.);
	...
	glTranslatef(10.*cos(2 * M_PI*Time), 0., 10.*sin(2 * M_PI*Time));
	...
	SetPointLight(GL_LIGHT2, 0., -5., 0., 1., 1., 1.);
    glPushMatrix();
	if (Light2On == 1)
		glEnable(GL_LIGHT2);
	else
	...
	SetMaterial(0.5, 0.5, 1.0, 100.);
	//glColor3f(1., 1., 0.);
	glutSolidTorus(0.8, 2., 100, 20);
	glPopMatrix();

	//Teapot
	glPushMatrix();
	glShadeModel(GL_SMOOTH);
	SetMaterial(0.8, 0.4, 0.0, 100.0);
	//glColor3f(1., 0., 0.);
	glutSolidTeapot(2.);
	glPopMatrix();
	...
	glDisable(GL_LIGHTING);
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., "Project4 - YuWenTseng" );
